...
`src/regions.json` is now generated by `tools/find_data.py` (`make regions`):
**117 data regions**. The window-descriptor records (`WindowOne`/`WindowTwo`/
`SelectedWind`) are carved (confirmed by `GetSelWind` copying their first 4
bytes into `WindowLft..WindowBot`). **109 inline message strings** are carved
with exact boundaries via the `jsr PrintMessage` ($5968) idiom — every message
is `20 68 59` followed by a `$00`-terminated high-bit string (that recurring
`hY` in the gloss was the JSR bytes). Round-trips green.

### Needs Brian's confirmation / open questions
- **Indexed string tables still inside code spans** (identified, not yet
  carved — they're table-structured, not `$00`-inline, so bounds need a careful
  pass): minor/major mode names ~`$54ED` ("Fundamental Text Merlin Lisp C
  Pascal…"), key names ~`$590C` ("Return Tab Escape Delete…"), month names
  `$6E79` ("Jan…Dec"), file-type names `$7655`, and the large variable-name
  table `$9755–$9A66`. Plus the binary dispatch tables `ComTab $4F84`,
  `CharIndex $3F6D`, `CompList $5987`.
