...
- Stage 5 (comments/polish) — substantial: structural region headers,
  ...
  data, `$00`/`$FF` fill collapsed to `ds`, and bank-switch notes. Operand
  naming is now **64%** (symbols + local labels). A full rationale pass still
  waits on Brian's behavioral labels for the anonymous routines.

### Data carving (Stage 4 continued)
`src/regions.json` is generated by `tools/find_data.py` (`make regions`):
**129 data regions** covering ~7.6 KB. 40% of address operands now carry names
and the count of bogus data-as-code instructions keeps dropping as data is
carved. Carved, all evidence-based and bounded by the next routine symbol:

- Window-descriptor records `WindowOne`/`WindowTwo`/`SelectedWind` (confirmed by
  `GetSelWind` copying their first 4 bytes into `WindowLft..WindowBot`).
- **109 inline message strings** via the `jsr PrintMessage` ($5968) idiom —
  `20 68 59` then a `$00`-terminated high-bit string (the recurring `hY` gloss
  was the JSR bytes).
- Dispatch tables: `ComTab $4F84-$504D`, `CharIndex $3F6D` (3B),
  `CompList/CompOffset/CompCount $5987-$598B`.
- Name tables: `ModeNames $54F2` + `MinorNames $5528`; `KeyNames $5908`
  (8 key codes + 8 pointers + name strings); `MonthNames $6E79` (12×3);
...
- **AMACSStack $1035** — treated as a 1-byte saved-stack var (see `stx $1035`).
  Confirm.
- Remaining data still decoded as code is now mostly small scattered records and
  in-image `doc:` documentation strings (e.g. around `doc:ExitSafe $504E`);
  these can be carved opportunistically as labeling continues.
